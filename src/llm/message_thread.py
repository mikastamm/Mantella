...
class message_thread():
    ...
    """
    def __init__(self, config: ConfigLoader, initial_system_message: str | SystemMessage | None, action_manager:ActionAccessor) -> None:
        ...
        if isinstance(initial_system_message, str):
            initial_system_message = SystemMessage(initial_system_message, config, action_manager)
    ...
    def reload_message_thread(self, new_prompt: str, is_too_long: Callable[[list[Message], float], bool], percent_modifier: float):
        ...
        """
        result: list[Message] = []
        result.append(SystemMessage(new_prompt, self.__config, self.__action_manager))
